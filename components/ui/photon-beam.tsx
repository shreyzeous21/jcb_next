...
export default function PhotonBeam(props: PhotonBeamProps = {}) {
  ...
  useEffect(() => {
    ...
    const init = (): void => {
      ...
      // --- CONFIGURATION ---
      const params: Params = {
        transparentBg: props.transparentBg ?? false,
        colorBg: props.colorBg ?? "#080808",
        colorLine: props.colorLine ?? "#080808",
      ...
      function animate(): void {
        ...
        if (params.transparentBg && renderer) {
      ...
      // Resize Handler
      handleResize = (): void => {
        if (!container || cancelled || !renderer) return;
  ...
  return (
    <div
      ref={containerRef}
      className={`h-full min-h-[200px] w-full ${props.transparentBg ? "bg-transparent" : "bg-black"}`}
    />
  );
}
